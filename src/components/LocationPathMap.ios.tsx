...
/*
 * ================================================================
 * LOCATION PATH MAP (iOS)
 * ================================================================
 *
 * Metro resolves this ahead of the shared LocationPathMap.tsx
 * fallback when bundling for iOS (`.ios.tsx` > `.native.tsx` >
 * `.tsx`). Android gets LocationPathMap.android.tsx; web gets
 * LocationPathMap.web.tsx (Leaflet). This brings iOS to the same
 * parity: real map tiles, the same green/blue/red numbered points,
 * the same START/END pill labels, the same popup fields, fit-to-
 * bounds on the same GPS data.
 *
 * Unlike Android, react-native-maps on iOS defaults (PROVIDER_
 * DEFAULT) to Apple Maps -- no Google Maps SDK and no API key
 * required. The OSM UrlTile layer below is drawn on top of the
 * Apple Maps base, which keeps the visual style consistent with
 * the web/Android versions without needing any credentials.
 */
  ...
  return (
    <View style={styles.container}>
      ...
      >
         ...
         * overlaid on the Apple Maps base)
         * ----------------------------------------------------------
         */}
        <UrlTile
